...
- 设备与配件管理：添加、编辑、删除、排序，支持批量添加多种附件和数量。
- 日均成本计算：设备本体日均、配件日均、总投入、合计日均、等效月租、等效年化成本。
...
- 模型图表：日均排行、投入构成、目标等待时间、未来日均曲线。
- 图表筛选：可选择单个设备、多个设备，并查看已选设备组合的未来日均合计曲线。
- 本地 AI 分析：调用本机 Ollama 的 `qwen3:8b`，基于本地统计数据生成分析建议。
